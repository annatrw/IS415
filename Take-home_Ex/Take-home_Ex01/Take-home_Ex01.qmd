...
Manually assign values to the duplicated values
...
Filter and extract the LGAs in Osun state, saving into a variable `osunT`.
...
There are 4 different kernel bandwidth estimation functions available in spatstat: `bw.diggle()`, `bw.ppl()`, `bw.scott()`, `bw.CvL()`; with the first 2 are recommended. `bw.diggle()` is appropriate for visualising one tight cluster amid random noise, while `bw.ppl()` is more appropriate for visualising data with predominantly tight clusters (read more <a href="https://maczokni.github.io/crime_mapping_textbook/studying-spatial-point-patterns.html">here</a>). 
...
The sselected 4 LGAs for this section of analysis are: Osogbo (high concentration of water points), Ede North (high concentration of water points), Ife North (low concentration of water points) and Ife South (low concentration of water points).
