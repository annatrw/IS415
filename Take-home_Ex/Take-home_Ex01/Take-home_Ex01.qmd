...
wp_osun\$wp_status \<- wp_osun %\>% select(`#status_clean`) %\>% mutate(wp_osun, if_else(`#status_clean`=="Functional", "Functional", "Non-Functional", "Unknown"))\
wp_osun
...
code not working rbind(osun_functional, osun_nonfunctional)

```{r}
tmap_mode("plot")
```

functional and non on the same map code not working
...
##Data preparation spatial class
...
## Selection of study areas for second order
...
left join
```{r}
osunT_join_wp <- st_join(osunT, wp_osun,
                          by = c("#clean_adm2" = "ADM2_EN"))
```

https://www.ijera.com/papers/Vol4_issue12/Part%20-%201/P04120194104.pdf
osogbo - low density
Ola-oluwa - medium residential density


```{r}
A = osun_spatial[osun_spatial@data$ADM2_EN == "Aiyedade",]
B = osun_spatial[osun_spatial@data$ADM2_EN == "Egbedore",]
C = osun_spatial[osun_spatial@data$ADM2_EN == "Ife Central",]
...
```{r}
par(mfrow=c(2,2))
plot(A, main = "Aiyedade")
plot(B, main = "Egbedore")
plot(C, main = "Ife Central")

```
```{r}
A_sp = as(A, "SpatialPolygons")
B_sp = as(B, "SpatialPolygons")
C_sp = as(C, "SpatialPolygons")
```
...
```{r}
A_ppp = functional_ppp[A_owin]
B_ppp = functional_ppp[B_owin]
C_ppp = functional_ppp[C_owin]
```
...
## Functional water points
h0 [null hypothesis] = functional water points are randomly distributed in Osun state

h1 = functional water points are not randomly distributed in Osun state

if p value < 0.05 [alpha value/ test statistic at 95% confidence level] = reject null hypothesis [which assumes a random distribution

if p > 0.05 = insufficient evidence to reject null hypothesis 

### using L function
```{r}
L_functional = Lest(functionalOsun_ppp, correction = "Ripley")
plot(L_functional, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)")
```
### Complete Spatial Rnadomness test

```{r}
L_functional.csr <- envelope(functionalOsun_ppp, Lest, nsim = 39, rank = 1, glocal=TRUE)
```
...
- L function is more than 0, signs of clustering 
- 
 

## Non-functional water points

h0 [null hypothesis] = non-functional water points are randomly distributed in Osun state

h1 = non-functional water points are not randomly distributed in Osun state

```{r}
L_nonfunctional = Lest(nonfunctionalOsun_ppp, correction = "Ripley")
plot(L_nonfunctional, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)")
```


L_nonfunctional.csr <- envelope(nonfunctionalOsun_ppp, Lest, nsim = 39, rank = 1, glocal=TRUE)
