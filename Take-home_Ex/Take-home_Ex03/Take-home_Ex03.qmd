...
```{r}
pacman::p_load(sf, tidyverse, tmap, jsonlite, rvest, onemapsgapi, olsrr, corrplot, ggpubr, spdep, GWmodel, gtsummary, ggthemes, zoo, SpatialML, tmap, rsample, Metrics)
```
...
We can observe the right skewed data from 'WITHIN_350M_KINDERGARTEN', while the other 3 factors are relatively even.
...
Various methods and models were used to derive a predictive model for HDB 5 room resale flats in Singapore based on independent variables:
...
The GWR model had performed better than the global regression model,
