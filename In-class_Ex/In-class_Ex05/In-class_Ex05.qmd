...
```
Searching for the 6 nearest neighbours (using adaptive kernel density method)
- choosing 6 nearest since the target point is included in function as well (total 7 and will not get an even split)
```{r}
...
A is target, B is neighbour category at 50 simulations 
outputs the p value directly
```{r}
